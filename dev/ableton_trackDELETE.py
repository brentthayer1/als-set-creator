...
class AbletonTrack(object):
    ...
    def __init__(self, track_root: ET.Element, version: Tuple[int, int, int]):
        ...
        self.unfolded = get_element(
            track_root, "TrackUnfolded", attribute="Value", silent_error=True
        )  # Ableton 10
        if not self.unfolded:
            folded = get_element(
                track_root, "DeviceChain.Mixer.IsFolded", attribute="Value"
            )  # Ableton 9/8
    ...
    def clip_arangement_colors(self):
